...
public class Job3Ver2Reducer extends  Reducer<Text, Text, Text, Text> {
	...
	private Map<String, List<String>> commonProducts = new TreeMap<String, List<String>>();
		...
		for (int i = 0; i < valueList.size(); i++) {
			...
			for (int j = i+1; j < valueList.size(); j++) {
				...
				if(commonProducts.get(orderedCouple.toString()) == null) {
					commonProducts.put(orderedCouple.toString(), new ArrayList<String>());
				}
	...
	InterruptedException {
		for (Map.Entry<String, List<String>> entry : commonProducts.entrySet()) {
